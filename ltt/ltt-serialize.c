...
static inline size_t serialize_trace_data(struct rchan_buf *buf,
		size_t buf_offset,
		char trace_size, enum ltt_type trace_type,
		char c_size, enum ltt_type c_type, int align,
				...
				const char *fmt, va_list *args)
{
	...
	cb(buf, buf_offset, closure, serialize_private, align, NULL, fmt, args);
}
		...
		const char *fmt, va_list *args)
{
	...
	list_for_each_entry_rcu(trace, &ltt_traces.head, list) {
		...
		ltt_write_event_data(buf, buf_offset, &closure,
					serialize_private,
					align, largest_align, fmt, &args_copy);
		va_end(args_copy);
		/* Out-of-order commit */
