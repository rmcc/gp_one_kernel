...
// find command line via pid, jones ++
...
//++ jones
        ...
        {
            //printk(KERN_DEBUG "[LTT]Ltt User thread\n");            
            // sprintf(cmdline, "/proc/%d/cmdline", t->pid); 
            memset( cmdline, 0, sizeof(cmdline) );
            proc_pid_cmdline(p, cmdline);
            // sprintf( cmdline, "%s:%s", t->comm, cmdline );
            //printk(KERN_DEBUG "pid:%d, name:%s\n", t->pid, cmdline);	
            __trace_mark(0, list_process_state, call_data,
				"pid %d parent_pid %d name %s type %d mode %d "
				"submode %d status %d tgid %d",
				t->pid, t->parent->pid, cmdline,
				type, mode, submode, status, t->tgid);
        }
        else
        {
            //printk(KERN_DEBUG "[LTT]Ltt Kernel thread\n");	
            //printk(KERN_DEBUG "pid:%d, name:%s\n", t->pid, t->comm);            
