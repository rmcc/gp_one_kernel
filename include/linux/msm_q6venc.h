...
struct q6_init_config {
	...
	unsigned short processing_type;
	...
	unsigned short ise_mv_cost_fac;
	...
	unsigned int enc_frame_buf1_phy;
	unsigned int enc_frame_buf2_phy;
	unsigned int enc_frame_num_words;
};

struct init_config {
	struct venc_buf ref_frame_buf1;
	struct venc_buf ref_frame_buf2;
	struct venc_buf enc_frame_buf1;
	struct venc_buf enc_frame_buf2;
...
struct q6_encode_param {
	...
	unsigned int encoded_pkt_buf_size;
...
struct q6_intra_refresh {
	unsigned short num_intra_mb;
	unsigned int mb_index_array_phy;
	int mb_index_array_len;
	int mb_index_array_len_req;
};

struct intra_refresh {
	struct venc_buf mb_index_array;
