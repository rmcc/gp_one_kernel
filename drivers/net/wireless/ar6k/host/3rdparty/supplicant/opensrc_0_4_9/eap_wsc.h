...
typedef struct wsc_notify_buildreq_tag {
    u32    id;
    u32 state;
} __ATTRIB_PACK WSC_NOTIFY_BUILDREQ;

typedef struct wsc_notify_process_buildreq_result_tag {
	u8 result;
} __ATTRIB_PACK WSC_NOTIFY_BUILDREQ_RESULT;

typedef struct wsc_notify_process_tag {
	u32 state;
} __ATTRIB_PACK WSC_NOTIFY_PROCESS;

typedef struct wsc_notify_process_result_tag {
	u8 result;
	u8 done;
} __ATTRIB_PACK WSC_NOTIFY_PROCESS_RESULT;
