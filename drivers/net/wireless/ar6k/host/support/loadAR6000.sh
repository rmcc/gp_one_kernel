...
TARGET_ONLY="FALSE"
# ATHENV
TGT_FW=${WORKAREA}/target/AR6002/hw2.0/bin/athwlan.bin.z77
TGT_PATCH=${WORKAREA}/target/AR6002/hw2.0/bin/data.patch.hw2_0.bin
TCMD_TGT_FW=$WORKAREA/target/AR6002/hw2.0/bin/athtcmd_ram.bin
# ATHENV
...
case $ATH_PLATFORM in
	...
	# ATHENV
	QUALCOMM_ARM-SDIO)
	/sbin/insmod $IMAGEPATH/sdio_lib.ko debuglevel=7
	/sbin/insmod $IMAGEPATH/sdio_busdriver.ko debuglevel=7
	/sbin/insmod $IMAGEPATH/sdio_qualcomm_hcd.ko debuglevel=7
	;;
	# ATHENV
	*)
	if [ -e "$ALTERNATE_PLAT_SCRIPT" ]; then
...
case $ATH_PLATFORM in
	# ATHENV
	LOCAL_i686-SDIO|LOCAL_i686-CF|SANDGATEII_ARM-SDIO|SANDGATEII_ARM-CF|SGDK020_2-MSIO|QUALCOMM_ARM-SDIO)
	/sbin/insmod $IMAGEPATH/ar6000.ko bmienable=1 $ar6000args busspeedlow=$slowbus tgt_fw="${TGT_FW}" tgt_patch="${TGT_PATCH}"
