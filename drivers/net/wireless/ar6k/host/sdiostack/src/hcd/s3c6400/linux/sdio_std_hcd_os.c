...
MODULE_PARM_DESC(hcdattributes, "S3C6400 Host Attributes");
                                     ...
                                     PTEXT      pName)
{
    ...
    do {
        ...
        if (pHcInstance == NULL) {
            DBG_PRINT(SDDBG_ERROR, ("SDIO S3C6400 Host: CreateStdHcdInstance - no memory for instance\n"));
            break;
        }
                              ...
                              SDDMA_DESCRIPTION       *pADMADescrip)
{
    ...
    do { 
        ...
        if (!SDIO_SUCCESS((status = HcdInitialize(pHcInstance)))) {
            DBG_PRINT(SDDBG_ERROR, ("SDIO S3C6400 HOST: StartStdHcdInstance - failed to init HW, status =%d\n",status));  
            break;
        }    
        ...
        if (!(Flags & START_HCD_FLAGS_FORCE_NO_DMA)) {
            ...
            if (!(Flags & START_HCD_FLAGS_FORCE_SDMA) && 
                  ...
                  (pADMADescrip != NULL)) {
                DBG_PRINT(SDDBG_TRACE, ("SDIO S3C6400 HOST: StartStdHcdInstance - using Advanced DMA\n"));
                ...
                pHcInstance->Hcd.pDmaDescription = &pHcInstance->DmaDescription;
                       ...
                       (pSDMADescrip != NULL)) {
                DBG_PRINT(SDDBG_TRACE, ("SDIO S3C6400 HOST: StartStdHcdInstance - using Simple DMA\n"));
                    /* copy the DMA description for advanced DMA */
        ...
        if (IS_HCD_ADMA(pHcInstance)) {
            ...
            if (!SDIO_SUCCESS(status)) {
                DBG_PRINT(SDDBG_ERROR, ("SDIO S3C6400 Host : StartStdHcdInstance - failed to setup DMA buffer\n"));
                break;
            }
        }  
    ...
    if (SDIO_SUCCESS(status)) {
        DBG_PRINT(SDDBG_ERROR, ("SDIO S3C6400 Host - ready! \n"));
    } else {
            /* undo everything */
...
PSDHCD_INSTANCE RemoveStdHcdInstance(PSDHCD_CORE_CONTEXT pStdCore)
{
    ...
    DBG_PRINT(SDDBG_TRACE, ("+SDIO S3C6400 HCD: RemoveStdHcdInstance\n"));
    ...
    if (pHcInstanceToRemove != NULL) {
        DBG_PRINT(SDDBG_TRACE, (" SDIO S3C6400 HCD: Deinitializing 0x%X \n",(UINT)pHcInstanceToRemove));
...
static SDIO_STATUS SetupDmaBuffers(PSDHCD_INSTANCE pHcInstance)
{
    ...
    if (pHcInstance->Hcd.pDmaDescription->Flags & SDDMA_DESCRIPTION_FLAG_SGDMA) {
        ...
        pHcInstance->OsSpecific.pDmaBuffer =  dma_alloc_coherent(pHcInstance->Hcd.pDevice, 
                                                  ...
                                                  &pHcInstance->OsSpecific.hDmaBuffer, 
                                                  GFP_DMA);
        DBG_PRINT(SDDBG_TRACE, ("SDIO S3C6400 Host : SetupDmaBuffers - pDmaBuffer: 0x%X, hDmaBuffer: 0x%X\n",
                                (UINT)pHcInstance->OsSpecific.pDmaBuffer , (UINT)pHcInstance->OsSpecific.hDmaBuffer ));
        if (pHcInstance->OsSpecific.pDmaBuffer == NULL) {
            DBG_PRINT(SDDBG_ERROR, ("SDIO S3C6400 Host : SetupDmaBuffers - unable to get DMA buffer\n"));
            return SDIO_STATUS_NO_RESOURCES;
        }        
        return SDIO_STATUS_SUCCESS;
    } else {
        DBG_PRINT(SDDBG_TRACE, ("SDIO S3C6400 Host : SetupDmaBuffers - invalid DMA type\n"));
...
struct work_struct *work)
{
    ...
    DBG_PRINT(STD_HOST_TRACE_SDIO_INT, ("SDIO S3C6400 HOST: hcd_sdioirq_wqueue_handler \n"));
    if (!pHcInstance->ShuttingDown) {
...
SDIO_STATUS SetUpHCDDMA(PSDHCD_INSTANCE pHcInstance, PSDREQUEST pReq)
{
    ...
    DBG_PRINT(STD_HOST_TRACE_DATA, ("SDIO S3C6400 HOST SetUpHCDDMA (%s) DescCount:%d Blocks:%d, BlockLen:%d\n",
        IS_SDREQ_WRITE_DATA(pReq->Flags) ?  "TX" : "RX",
        pReq->DescriptorCount, pReq->BlockCount, pReq->BlockLen));
    
//    printk (KERN_ERR "SDIO S3C6400 HOST SetUpHCDDMA (%s) DescCount:%d Blocks:%d, BlockLen:%d\n",
//        IS_SDREQ_WRITE_DATA(pReq->Flags) ?  "TX" : "RX",
//        pReq->DescriptorCount, pReq->BlockCount, pReq->BlockLen);
    ...
    if (IS_HCD_ADMA(pHcInstance)) {
        ...
        for (ii = 0; ii < pReq->DescriptorCount; ii++,pDescriptor++) {
            DBG_PRINT(STD_HOST_TRACE_DATA, ("SDIO S3C6400 HOST SetUpHCDDMA ADMA Descrp: 0x%X, ReqDescrip: 0x%X, len: %d bytes, addr: 0x%X\n",
        ...
        DBG_PRINT(STD_HOST_TRACE_DATA, ("SDIO S3C6400 HOST SetUpHCDDMA SDMA, ReqDescrip: 0x%X, len: %d bytes, addr: 0x%X\n",
    ...
    if (totalLength != (pReq->BlockCount * pReq->BlockLen)) {
        DBG_PRINT(SDDBG_ERROR, ("SDIO S3C6400 HOST DMA Block Length and Count Mismatch SGList Reports:%d bytes, Bus request : Blocks:%d,Bytes Per Block:%d \n",
...
{
    if (IS_HCD_ADMA(pHcInstance)) {
        DBG_PRINT(SDDBG_ERROR, ("SDIO S3C6400 HOST, ADMA Descriptor Start (PHYS):0x%X \n",
                 (UINT32)pHcInstance->OsSpecific.hDmaBuffer));    
        SDLIB_PrintBuffer((PUCHAR)pHcInstance->OsSpecific.pDmaBuffer, SDHCD_ADMA_DESCRIPTOR_SIZE, "SDIO S3C6400 HOST: ALL DMA Descriptors");                           
    }
}
...
BOOL HandleSharedStdHostInterrupt(PSDHCD_CORE_CONTEXT pStdCore)
{
    ...
    DBG_PRINT(STD_HOST_TRACE_INT, ("SDIO S3C6400 HOST HandleSharedStdHostInterrupt : slot ints:0x%X \n",interruptingSlots));
    ...
    SDITERATE_OVER_LIST(&pStdCore->SlotList, pListItem) {
        ...
        if ((1 << pHcInstance->OsSpecific.SlotNumber) & interruptingSlots) {
            DBG_PRINT(STD_HOST_TRACE_INT, ("SDIO S3C6400 HOST HandleSharedStdHostInterrupt pHcInstance: 0x%X, slot:%d is interrupting \n",
...
SDIO_STATUS StartStdHostCore(PSDHCD_CORE_CONTEXT pStdCore)
{
    ...
    do {
        ...
        SDITERATE_OVER_LIST(&pStdCore->SlotList, pListItem) {
            ...
            if (!SDIO_SUCCESS(status)) {
                DBG_PRINT(SDDBG_ERROR, ("SDIO S3C6400 Host - failed to register with host, status =%d\n",status));
                break;    
            }
