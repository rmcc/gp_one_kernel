...
INT sd0_dat0_gpio_pin = -1;
module_param(sd0_dat0_gpio_pin, int, 0444);
MODULE_PARM_DESC(sd0_dat0_gpio_pin, "slot 0 DAT0 GPIO polling pin number");

INT sd0_dat0_gpio_pad_conf_offset = 0;
module_param(sd0_dat0_gpio_pad_conf_offset, int, 0444);
MODULE_PARM_DESC(dat0_gpio_pad_conf_offset, "slot 0 DAT0 GPIO polling pin pad config offset");

INT sd0_dat0_gpio_pad_conf_byte = 0;
module_param(sd0_dat0_gpio_pad_conf_byte, int, 0444);
MODULE_PARM_DESC(sd0_dat0_gpio_pad_conf_byte, "slot 0 DAT0 GPIO pin pad byte number");

INT sd0_dat0_gpio_pad_mode_value = 0;
module_param(sd0_dat0_gpio_pad_mode_value, int, 0444);
MODULE_PARM_DESC(sd0_dat0_gpio_pad_mode_value, "slot 0 DAT0 GPIO pin mode value");

/* Example DAT0 gpio polling settings if GPIO pin used is GPIO_16:
 * 
 *  sd0_dat0_gpio_pin=16
 *  sd0_dat0_gpio_pad_conf_offset=232 (0x00E8)   - pad control register offset
 *  sd0_dat0_gpio_pad_conf_byte=0          - byte index into register (each register configure 4 pins)   
 *  sd0_dat0_gpio_pad_mode_value=3         - mode value to switch pin to GPIO mode (multiplexed)
 * 
 */
...
#endif
{
    ...
    for (ii = first_device; ii < device_count+first_device; ii++) {
        ...
        pDeviceContext->OSInfo.DAT0_gpio_pin = -1;
        
        if (ii == 0) {
                /* for slot 0, pass the gpio pin settings for polling DAT0 (WAR for the DAT0-busy errata) */
            pDeviceContext->OSInfo.DAT0_gpio_pin = sd0_dat0_gpio_pin;
            pDeviceContext->OSInfo.DAT0_gpio_conf_offset = sd0_dat0_gpio_pad_conf_offset;
            pDeviceContext->OSInfo.DAT0_gpio_pad_conf_byte = sd0_dat0_gpio_pad_conf_byte;
            pDeviceContext->OSInfo.DAT0_gpio_pad_mode_value = sd0_dat0_gpio_pad_mode_value;
        }
