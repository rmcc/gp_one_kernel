...
{
    /* nothing to do, just free request */
    kfree(pReq);
}
...
static void gps_stop_tx(struct uart_port *port, unsigned int tty_stop)
{
    ...
    if (pDevice->InterruptEnable & UART_ETBEI) {
        ...
        /* we need to manage our own request structs becuase we are called with ints disabled */
        pReq = kmalloc(sizeof(SDREQUEST),  GFP_ATOMIC);
        if (pReq != NULL) {
            WriteRegisterAsynch(pDevice, pReq, TxCompletion, UART_INT_ENABLE_REG, pDevice->InterruptEnable);
        } else {
            DBG_PRINT(SDDBG_ERROR, ("SDIO GPS Function: out of memory\n"));
        }
