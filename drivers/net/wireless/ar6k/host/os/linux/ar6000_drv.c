 ...
 * WLAN Device
 */
static const char athId[] __attribute__ ((unused)) = "$Id: //depot/sw/releases/olca2.1-RC/host/os/linux/ar6000_drv.c#11 $";
...
static char *tgt_patch = "/system/etc/data.patch.hw2_0.bin";
///char *tgt_fw = NULL;
///static char *tgt_patch = NULL;
///---FIH---
...
/* ATHENV */
extern void eeprom_ar6000_transfer(HIF_DEVICE *device);
...
/* ATHENV */
    if (tgt_fw != NULL) {
        eeprom_ar6000_transfer(ar->arHifDevice);
        if ((tgt_fw[strlen(tgt_fw) - 3] == 'z')
         && (tgt_fw[strlen(tgt_fw) - 2] == '7')
         && (tgt_fw[strlen(tgt_fw) - 1] == '7')) {
            firmware_transfer(ar->arHifDevice, tgt_fw, 0x502400, TRUE);
	} else {
            firmware_transfer(ar->arHifDevice, tgt_fw, 0x502400, FALSE);
        }
        firmware_transfer(ar->arHifDevice, tgt_patch, 0x52d8bc, FALSE);
        {
            A_UINT32 value;
            A_STATUS ret;

            BMIWriteSOCRegister(ar->arHifDevice, 0x500418, 0x52d8bc);
