...
ar6000_scan_node(void *arg, bss_t *ni)
{
    ...
    current_ev = iwe_stream_add_event(param->info,current_ev, end_buf, &iwe,
                                      IW_EV_ADDR_LEN);
    ...
    current_ev = iwe_stream_add_point(param->info,current_ev, end_buf, &iwe,
                                      &cie->ie_ssid[2]);
    ...
    if (cie->ie_wpa != NULL) {
        ...
        current_ev = iwe_stream_add_point(param->info,current_ev, end_buf, &iwe, buf);
    }
...
#define CHAN_IS_11A(x)              (!((x >= 2412) && (x <= 2484)))
    ...
    current_ev = iwe_stream_add_event(param->info,current_ev, end_buf, &iwe, IW_EV_CHAR_LEN);
