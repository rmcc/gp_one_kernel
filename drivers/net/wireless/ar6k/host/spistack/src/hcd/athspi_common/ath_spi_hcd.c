...
@abstract: Common Atheros-spi protocol handling code

#notes: OS independent code
 
@notice: Copyright (c), 2005-2007 Atheros Communications Inc.
...
static void DumpRequest(PSDREQUEST pReq) 
{
    ...
    DBG_PRINT(SDDBG_ERROR, ("************************REQUEST DUMP*********************** \n"));
             
    do {
        
        if (ATH_IS_TRANS_PIO(pReq)) {
            
            DBG_PRINT(SDDBG_ERROR, ("PIO Request %s , %s , Address : 0x%4.4X\n",
            ...
            if (ATH_IS_TRANS_PIO_INTERNAL(pReq)) {
                if (ATH_IS_TRANS_WRITE(pReq)) { 
                    DBG_PRINT(SDDBG_ERROR, ("INTERNAL WRITE data: 0x%4.4X   \n",ATH_GET_PIO_WRITE_VALUE(pReq)));
                }
                break;
            }
            
            DBG_PRINT(SDDBG_ERROR, ("EXTERNAL Access : %d  bytes  buffer:0x%X \n",
        ...
        DBG_PRINT(SDDBG_ERROR, ("DMA Request %s , Address : 0x%4.4X\n",
                    ATH_IS_TRANS_READ(pReq) ? "READ" : "WRITE",
                    ATH_GET_IO_ADDRESS(pReq)));
                    
        DBG_PRINT(SDDBG_ERROR, ("   %d  bytes  buffer:0x%X \n",
            ...
            if (IS_SLEEP_WAR_ENABLED(pDevice)) { 
                ...
                if (pDevice->WriteBufferSpace < pReq->DataRemaining) {
                    ...
                    DoPioReadInternal(pDevice,
                                      ATH_SPI_STATUS_REG,
                                      &spistatus);
                    DBG_PRINT(SDDBG_ERROR, ("*** ATH SPI - Error: WR_SPC:0x%4.4X STAT:0x%4.4X ***\n",
...
BOOL HcdSpiInterrupt(PSDHCD_DEVICE pDevice)
{
    ...
    do {
        ...
        if (interrupts & ATH_SPI_HCD_ERROR_IRQS) {
            PSDREQUEST pReq = GET_CURRENT_REQUEST(&pDevice->Hcd);
            DBG_PRINT(SDDBG_ERROR, ("ATH SPI - ERRORs detected in INTR_CAUSE : 0x%X \n", interrupts));
                                
            if (interrupts & ATH_SPI_INTR_ADDRESS_ERROR) {
                DBG_PRINT(SDDBG_ERROR, ("ATH SPI - ADDRESS ERROR \n")); 
            }
            
            if (interrupts &  ATH_SPI_INTR_WRBUF_ERROR) {
                DBG_PRINT(SDDBG_ERROR, ("ATH SPI - WRBUF ERROR \n"));     
            }
            
            if (interrupts & ATH_SPI_INTR_RDBUF_ERROR) {
                DBG_PRINT(SDDBG_ERROR, ("ATH SPI - RDBUF ERROR \n"));     
            }
