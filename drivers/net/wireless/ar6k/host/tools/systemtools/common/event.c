...
#ident  "ACI $Id: //depot/sw/releases/olca2.1-RC/host/tools/systemtools/common/event.c#1 $, $Header: //depot/sw/releases/olca2.1-RC/host/tools/systemtools/common/event.c#1 $"

#ifndef AR6K
static  char *rcsid =  "ACI $Id: //depot/sw/releases/olca2.1-RC/host/tools/systemtools/common/event.c#1 $, $Header: //depot/sw/releases/olca2.1-RC/host/tools/systemtools/common/event.c#1 $";
#endif
...
EVENT_QUEUE *initEventQueue
    ...
    initializeLock(&(pQueue->queueLock));
    ...
    )
{
 ...
 if(protect)
        {
		releaseLock(pQueue->queueLock);
	}

	deleteLock(pQueue->queueLock);
...
{	
	...
	initializeLock(&(eventArray->arrayLock));	
}
...
EVENT_STRUCT *getEvent(EVENT_ARRAY * eventArray,A_BOOL protect)
{
	...
	if(protect){
		acquireLock(eventArray->arrayLock);
	}   
	...
	if (index < EVENT_ARRAY_SIZE)
	{
		...
		if(protect) {
			releaseLock(eventArray->arrayLock);
		}
...
void freeEvent(EVENT_STRUCT * event,A_BOOL protect) {
	...
	EVENT_ARRAY * eventArray=event->eventArrayPtr;
...
A_UINT16 pushEvent
    ...
    /* wait for the mutex, as want mutual exclusive access */
 if(protect)
        {
		acquireLock(pQueue->queueLock);
	}
