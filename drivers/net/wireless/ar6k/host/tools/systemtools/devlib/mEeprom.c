...
#ident  "ACI $Id: //depot/sw/releases/olca2.1-RC/host/tools/systemtools/devlib/mEeprom.c#2 $, $Header: //depot/sw/releases/olca2.1-RC/host/tools/systemtools/devlib/mEeprom.c#2 $"
...
#if 0
        printf(" eepData pwrIn %d, numPdG %d, midPwr %d, xpdGainVals %d %d %d %d\n", powerIn,
            pLibDev->eepData.numPdGain, pLibDev->eepData.midPower, pLibDev->eepData.xpdGainValues[0],
            pLibDev->eepData.xpdGainValues[1], pLibDev->eepData.xpdGainValues[2], pLibDev->eepData.xpdGainValues[3]);
#endif
...
)
{
	...
	if(twicePower >= PWR_TABLE_SIZE) {	
		if (twicePower == 0xFF) {
			...
			mError(devNum, EINVAL, "getPowerIndex : devNum%d, twicePower %d should be less than %d\n", devNum, twicePower, PWR_TABLE_SIZE);
			return(0xdead);
		}
	}
...
)
{
	...
	// for ch14 in CCK mode, which outputs 0.5dB below target
	
	if( (dataRate > 7) & ((A_UINT)freq == 2484) )
	{
		ratesArray[dataRate]++;
	}
