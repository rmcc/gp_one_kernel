...
#ident  "ACI $Id: //depot/sw/releases/olca2.1-RC/host/tools/systemtools/devlib/mAlloc.c#1 $, $Header: //depot/sw/releases/olca2.1-RC/host/tools/systemtools/devlib/mAlloc.c#1 $"
...
//	const A_UINT32 beefy = 0x0;
   ...
   //printf("SNOOP::memAlloc (%d):called\n", numBytes); 
...
)
{
 A_UINT32 blockIndex;
	LIB_DEV_INFO *pLibDev = gLibInfo.pLibDevArray[devNum];
	for (blockIndex = 0; blockIndex < pLibDev->mem.allocMapSize; blockIndex++) 
	  memMarkIndexesFree(devNum, blockIndex);
}
