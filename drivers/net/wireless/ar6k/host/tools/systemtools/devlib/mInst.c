...
MANLIB_API double tempMeasTemp(const unsigned long ud) {
    ...
    //if ( rsp[pos] == 'N' ) return 0;
        ...
        point = pow((double)10, (double)(-1 * (rsp[pos + 2] & 0x3)) );  //make this type convert to meet VS 2005
...
MANLIB_API double spaMeasChannelPower(const int ud, 
    ...
    for (i = 0; i < iSwpPnts ; i++) {
        sum += pow(10.0,(double)(trace[i]/10));
// _IQV        sum += pow(10,(trace[i]/10));
        n++;
    }
    avg = sum / n;
    scale = int_bw / (rbw * kn);
...
double spa_calc_obwl(double *trace, const double tot) {
    ...
    for (m = 0; m < 200; m++) {
        sum += pow(10.0,(double)(trace[m]/10));
// _IQV        sum += pow(10,(trace[m]/10));
        if (sum >= tot) {
            sum_last = sum - pow(10.0,(double)(trace[m]/10));
// _IQV            sum_last = sum - pow(10,(trace[m]/10));
...
double spa_calc_obwu(double *trace, const double tot) {
    ...
    for (k = 0; k < n; k++) {
        sum += pow(10.0,(double)(trace[m-k]/10));
// _IQV        sum += pow(10,(trace[m-k]/10));
        if (sum >= tot) {
            sum_last = sum - pow(10.0,(double)(trace[m-k]/10));
// _IQV            sum_last = sum - pow(10,(trace[m-k]/10));
...
extern double *spa_get_trace(const int ud) {
    ...
    switch(spa_model) {
         ...
         case SPA_E4404B :
            ...
            if (cToken != NULL) dResult[lCount] = atof(cToken);
            else dResult[lCount] = 0;
