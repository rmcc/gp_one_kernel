...
/* ATHENV */
        if ((TargetVersion == AR6002_VERSION_REV2) && (tgt_fw != NULL)) {
	    printk("%s: write 0x180c0 = OR 0x8\n", __FUNCTION__);
            status = BMIReadSOCRegister(hifDevice,0x180c0,&value);

            if (A_FAILED(status)) {
		printk("%s: BMIReadSOCRegister : 0x180c0 failed, status=%d\n", __FUNCTION__, status); 
                break;
            }

            value |= 0x8;

            status = BMIWriteSOCRegister(hifDevice,0x180c0,value);

            if (A_FAILED(status)) {
		printk("%s: BMIWriteSOCRegister : 0x180c0 failed, status=%d\n", __FUNCTION__, status); 
                break;
            }

            printk("%s: write 0x40c4 = OR 0x1\n", __FUNCTION__);
            ...
            status = BMIReadMemory(hifDevice,
                                   HOST_INTEREST_ITEM_ADDRESS(TARGET_TYPE_AR6002, hi_ext_clk_detected),
                                   (A_UCHAR *)&value,
                                   4);
            if (A_FAILED(status)) {
                printk("%s: BMIReadMemory addr=0x%x failed, status=%d\n",
                       __FUNCTION__, 
                       HOST_INTEREST_ITEM_ADDRESS(TARGET_TYPE_AR6002, hi_ext_clk_detected),
                       status); 
                break;
