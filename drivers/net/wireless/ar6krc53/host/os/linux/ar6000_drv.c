 ...
 * WLAN Device
 */
static const char athId[] __attribute__ ((unused)) = "$Id: //depot/sw/releases/olca2.2/host/os/linux/ar6000_drv.c#9 $";
/* ATHENV */
#include <linux/fs.h>
...
char *tgt_fw = "/system/etc/athwlan.bin.z77";
char *tgt_patch = "/system/etc/data.patch.hw2_0.bin";
char *tcmd_fw = "/system/etc/athtcmd_ram.bin";
static char *art_fw = "/system/etc/device.bin";
char *eeprom_bin = "/system/etc/eeprom.bin";
...
#error "define FAKE_EEPROM_USED"
char *eeprom_data = "/system/etc/fakeBoardData_AR6002.bin";
...
void COEX_SET_BT_STATE(u8 streamType,u8 status,WMI_SET_BT_PARAMS_CMD *pParam)
{
    AR_SOFTC_T *ar;

    if(ar6000_devices[0] == NULL){
        printk(KERN_ERR "%s Ar6k: COEX_SET_BT_STATE device unload.\n",__func__);
        return;
    }

    ar = (AR_SOFTC_T *)(netdev_priv(ar6000_devices[0]));
    if(ar == NULL){
        printk(KERN_ERR "%s Ar6k: COEX_SET_BT_STATE AR_SOFTC is NULL.\n",__func__);
        return;
    }
    ...
    #ifdef DEBUG
    AR_DEBUG_PRINTF("Ar6k:COEX_SET_BT_STATE streamType=%d, status=%d.\n",streamType,status);
    if((streamType == BT_STREAM_SCO || streamType == BT_STREAM_A2DP) && 
       status == BT_STATUS_START    && 
       pParam != NULL) {
        AR_DEBUG_PRINTF("    paramtype : %d \n",pParam->paramType);
        if(streamType == BT_STREAM_SCO) {
            AR_DEBUG_PRINTF("    numScoCyclesForceTrigger : %d \n",pParam->info.scoParams.numScoCyclesForceTrigger);
            AR_DEBUG_PRINTF("    dataResponseTimeout      : %d \n",pParam->info.scoParams.dataResponseTimeout);
            AR_DEBUG_PRINTF("    stompScoRules            : %d \n",pParam->info.scoParams.stompScoRules);                
            AR_DEBUG_PRINTF("    stompDutyCyleVal         : %d \n",pParam->info.scoParams.stompDutyCyleVal);
            AR_DEBUG_PRINTF("    psPollLatencyFraction    : %d \n",pParam->info.scoParams.psPollLatencyFraction);
            AR_DEBUG_PRINTF("    noSCOSlots               : %d \n",pParam->info.scoParams.noSCOSlots);
            AR_DEBUG_PRINTF("    noIdleSlots              : %d \n",pParam->info.scoParams.noIdleSlots);     
        }
    }
    #endif
    ...
    if( (streamType == BT_STREAM_SCO || streamType == BT_STREAM_A2DP) && 
       ...
       pParam != NULL)
    {
        if( wmi_set_bt_params_cmd(ar->arWmi, pParam) != A_OK)
            printk("%s wmi_set_bt_params_cmd fail .\n",__func__);
    }*/

    if (wmi_set_bt_status_cmd(ar->arWmi, streamType, status) != A_OK)
        printk("%s wmi_set_bt_status_cmd fail .\n",__func__);

	return;
}

EXPORT_SYMBOL(COEX_SET_BT_STATE);
...
/* ATHENV */
    ...
    //FIH
    wake_lock_init(&ar6k_wlock, WAKE_LOCK_SUSPEND, "ar6k");
...
/* ATHENV V7.2 +++ */
extern void eeprom_ar6000_transfer(HIF_DEVICE *device, char *fake_file, char *p_mac);
...
ar6000_avail_ev(void *context, void *hif_handle)
{
    ...
    //[ADQ.FC-522]
    //not allow suspend if WIFI is initialing
    wake_lock(&ar6k_wlock);
...
/* ATHENV V7.2 +++ */
    ...
    if (ifname[0])
    {
        ...
        strcpy(dev->name, AR6K_DRV_IF_NAME);
    }
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0) */
...
/* ATHENV */
	...
	/* ATHENV V7.1 +++ */
        /* Set hi_refclk_hz */
        printk("Set hi_refclk_hz : Ref Clock=%d\n", refClock);	
...
// But we can change the eeprom data at the driver side
        printk("AR6000: eeprom transfer - HOST\n");
        ...
        if (work_mode == 2) {
            ...
            //---ADQ.B-3233 start a scan immediately if it's normal WIFI mode
...
//        if(request_firmware(NULL, "ar6k_firmware", &dev->dev)!=0) {
    ...
    //[ADQ.B-1422] enable BT/WIFI coexistence
    //set ANT as switch
    WMI_SET_BT_PARAMS_CMD   paramCmd;
    A_MEMZERO(&paramCmd,sizeof(paramCmd));
    paramCmd.paramType = BT_PARAM_ANTENNA_CONFIG;
    paramCmd.info.antType = 3;
    if (wmi_set_bt_params_cmd(ar->arWmi, &paramCmd) != A_OK)
         printk("%s wmi_set_bt_params_cmd ANT as switch fail.\n",__func__);
    
    //BTS 4020
    A_MEMZERO(&paramCmd,sizeof(paramCmd));
    paramCmd.paramType = BT_PARAM_COLOCATED_BT_DEVICE;
    paramCmd.info.coLocatedBtDev = 0;
    if (wmi_set_bt_params_cmd(ar->arWmi, &paramCmd) != A_OK)
         printk("%s wmi_set_bt_params_cmd COLOCATED_BT_DEVICE as BTS4020 fail.\n",__func__);
    
    //FIH send resume event to wpa_supplicant
    
    union iwreq_data wrqu;
    A_MEMZERO(&wrqu, sizeof(wrqu));
    wrqu.data.length = strlen("resume");
    wireless_send_event(ar->arNetDev, IWEVCUSTOM, &wrqu, "resume");
...
ar6000_destroy(struct net_device *dev, unsigned int unregister)
{
    ...
    if (ar->arWmiReady == TRUE)
    {
        if (!bypasswmi)
        {
            ...
            {
                //FIH send suspend event to wpa_supplicant
                union iwreq_data wrqu;
                A_MEMZERO(&wrqu, sizeof(wrqu));
                wrqu.data.length = strlen("suspend");
                wireless_send_event(ar->arNetDev, IWEVCUSTOM, &wrqu, "suspend");

                //FIH  report disconnect event
                A_MEMZERO(&wrqu, sizeof(wrqu));
...
ar6000_scanComplete_event(AR_SOFTC_T *ar, A_STATUS status)
{
    ...
    if (wmi_bssfilter_cmd(ar->arWmi, NONE_BSS_FILTER, 0) != A_OK) {
        printk("Couldn't set filtering to NONE_BSS_FILTER\n");
    }
}
...
//FIH
//don't send event to APP to improve performance
