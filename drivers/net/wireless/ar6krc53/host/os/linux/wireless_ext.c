              ...
              struct iw_point *erq, char *keybuf)
{
    ...
    //FIH+++ 
    //prevent wpa_supplicant change wep key fail
    /*
    if(ar->arNextMode != AP_NETWORK) {
        //
                 ...
                 union iwreq_data *wrqu, char *extra)
{
    ...
    if (wrqu->power.disabled) {
       ...
       //before DHCP request, android disable power saving mode
       //cancel scan to prevent DHCP lost packet
       wmi_abort_scan_cmd(ar->arWmi);
       ar->bSkipScan = TRUE;
    }else{
...
#define SCAN_INT                HOME_TXDRAIN_TIME + ACT_DWELLTIME_DEFAULT
    ...
    //FIH+++
    if(ar->arConnected  &&  ar->bSkipScan){     //DHCP requesting, skip scan
        return 0;
    }

    //probe specific ssid for hidden network
	if (req && (data->flags & IW_SCAN_THIS_ESSID) && req->essid_len) {
        if(++ar->probeIndex >= MAX_PROBED_SSID_INDEX)
           ar->probeIndex = 0;
        if( wmi_probedSsid_cmd(ar->arWmi, ar->probeIndex, SPECIFIC_SSID_FLAG,req->essid_len,req->essid)!= A_OK)
        {
            printk("Couldn't set probe ssid list\n");                           
            ret = -EIO;                                     
        }

        if (wmi_bssfilter_cmd(ar->arWmi, ALL_BSS_FILTER, 0) != A_OK) {
            printk("Couldn't set probe ssid\n");                           
            ret = -EIO;                                     
        }                                                   
        ar->scan_complete = 0;
        if (wmi_startscan_cmd(ar->arWmi, WMI_SHORT_SCAN /*WMI_LONG_SCAN*/, FALSE, FALSE, \
    ...
    //ar->scan_complete = 0;
    //FIH
    //maybe set back to NONE_BSS_FILTER somewhere
    /*
