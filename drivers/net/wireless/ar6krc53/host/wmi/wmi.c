...
wmi_bssInfo_event_rx(struct wmi_t *wmip, A_UINT8 *datap, int len)
{
    ...
    A_UINT8 ie_ssid[ie_ssid_buf_len];

    if (len <= sizeof(WMI_BSS_INFO_HDR)) {
        return A_EINVAL;
    }

    A_MEMZERO(ie_ssid, ie_ssid_buf_len);
    ...
    if (bss != NULL) {
        //FIH remember ssid 
        if(bss->ni_cie.ie_ssid) {
            if(bss->ni_cie.ie_ssid[1] > ie_ssid_buf_len - 2) {
                printk("WIFI WMI ie_ssid don't have enough size.\n");
            }else if(bss->ni_cie.ie_ssid[1] && bss->ni_cie.ie_ssid[2] > 1)
                A_MEMCPY(ie_ssid, bss->ni_cie.ie_ssid, bss->ni_cie.ie_ssid[1] + 2);
        } else {
            printk("WIFI WMI bss have null ie_ssid.\n");
        }
    ...
    //FIH copy ssid for hidden network
    if(bss->ni_cie.ie_ssid) {
        if( bss->ni_cie.ie_ssid[1] == 0 && ie_ssid[1] > 0){
            A_MEMCPY(bss->ni_hiddenSsid, ie_ssid, ie_ssid[1] + 2);
            bss->ni_cie.ie_ssid = bss->ni_hiddenSsid;
        }else if(bss->ni_cie.ie_ssid[1] == ie_ssid[1] && bss->ni_cie.ie_ssid[2] <= 1 && ie_ssid[1] > 0) {
            A_MEMCPY(&bss->ni_cie.ie_ssid[1], &ie_ssid[1], ie_ssid[1] + 1);
        }
    }else {
        printk("WIFI WMI get beacon without ssid\n");
        if(ie_ssid[1] > 0){
            A_MEMCPY(bss->ni_hiddenSsid, ie_ssid, ie_ssid[1] + 2);
            bss->ni_cie.ie_ssid = bss->ni_hiddenSsid;
        }
