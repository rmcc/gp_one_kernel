...
int ar6000_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
    ...
    {
//FIH+++
        case AR6000_XIOCTL_ANDROID_GET_RSSI:
        {
            int rssi = ar->arTargetStats.cs_rssi;      //cs_aveBeacon_rssi
            ...
            if (copy_to_user(rq->ifr_data,&rssi , sizeof (rssi)) ) 
            {
                ret = -EFAULT;
                printk("WIFI:Driver: AR6000_XIOCTL_ANDROID_GET_RSSI copy_to_user fail\n");
            }else
                ret = sizeof(rssi);
                        
            break;
        }
        case AR6000_XIOCTL_ANDROID_GET_LINKSPEED:
        {
            int linkSpeed = ar->arBitRate;

            if (ar->arWmiReady == FALSE || !ar->arConnected)
                ret = -EIO;
            ...
            if (copy_to_user(rq->ifr_data,&linkSpeed , sizeof (linkSpeed)) ) 
            {
                ret = -EFAULT;
                printk("WIFI:Driver: AR6000_XIOCTL_ANDROID_GET_LINKSPEED copy_to_user fail\n");
            }else
                ret = sizeof(linkSpeed);
                       
            break;
        }
        case AR6000_XIOCTL_ANDROID_GET_MACADDR:
        {
            if (ar->arWmiReady == FALSE)
                ret = -EIO;
  
            if (copy_to_user(rq->ifr_data,ar->arNetDev->dev_addr , 6) ) 
            {
                ret = -EFAULT;
                printk("WIFI:Driver: AR6000_XIOCTL_ANDROID_GET_LINKSPEED copy_to_user fail\n");
            }else
