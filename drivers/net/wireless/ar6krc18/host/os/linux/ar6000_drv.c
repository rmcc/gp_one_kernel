 ...
 * WLAN Device
 */
static const char athId[] __attribute__ ((unused)) = "$Id: //depot/sw/releases/olca2.2/host/os/linux/ar6000_drv.c#3 $";
/* ATHENV */
#include <linux/fs.h>
/* ATHENV */
...
static char *art_fw = "/system/etci/device.bin";
...
#ifdef DEBUG
//#error "debug build"
A_UINT32 g_dbg_flags = DBG_DEFAULTS;    //FIH+++    0xffff;  
unsigned int debugflags = 0;
int debugdriver =  1;    //FIH+++  3;
...
/* ATHENV */
    ...
    if (tgt_fw != NULL) {
        ...
        /* Set hi_refclk_hz */
        ///FIH+++
        value = 26000000; // for 26MHz
        //value = 19200000; // for 19.2MHz
        printk("Set hi_refclk_hz : value=%d\n", value);	
        BMIWriteSOCRegister(ar->arHifDevice, 0x500478, value);
...
// But we can change the eeprom data at the driver side
        ...
        eeprom_ar6000_transfer(ar->arHifDevice, eeprom_data);
#else 
...
#else
    pIwStats->qual.qual = pStats->cs_aveBeacon_rssi - 161; /* noise is -95 dBm */
