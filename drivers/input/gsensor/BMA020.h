...
/** \file bma020.h
    \brief Header file for all #define constants and function prototypes for native bma020 driver dll.
*/
...
*/
#define BMA020_WR_FUNC_PTR int (bus_write)(unsigned char , unsigned char, unsigned char *, unsigned char)
...
#define BMA020_RD_FUNC_PTR int ( bus_read)(unsigned char, unsigned char, unsigned char *, unsigned char)
...
#define BMA020_IRQ				0x07

/*
	BMA020 API error codes
*/

#define E_SMB_NULL_PTR		(char)-1
...
int bma020_get_mode(unsigned char*);
...
int bma020_read_accel_xyzt(bma020acc_t *);
...
int spi_bus_write(unsigned char dev_addr, unsigned char reg_addr, unsigned char * data, unsigned char datasize);
...
extern "C"{
	...
	__declspec(dllexport) DWORD SEN_Init(PVOID Context);
	__declspec(dllexport) BOOL SEN_Deinit(DWORD dwContext);
	__declspec(dllexport) ULONG SEN_Seek(PVOID Context,LONG Position,DWORD Type);
	__declspec(dllexport) DWORD SEN_Write(DWORD  dwContext,LPVOID pBuf,DWORD  Len) ;
	__declspec(dllexport) DWORD SEN_Read(DWORD  dwContext,LPVOID pBuf,DWORD  Len) ;
	__declspec(dllexport) BOOL  SEN_Close(DWORD Context) ;
	__declspec(dllexport) DWORD SEN_Open(DWORD Context, DWORD Access,DWORD ShareMode);
	__declspec(dllexport) VOID SEN_PowerUp(DWORD dwContext);
	__declspec(dllexport) VOID SEN_PowerDown(DWORD dwContext);
	__declspec(dllexport) BOOL SEN_IOControl(DWORD  dwContext,DWORD  Ioctl,PUCHAR pInBuf,DWORD  InBufLen, PUCHAR pOutBuf,DWORD  OutBufLen,PDWORD pdwBytesTransferred);
