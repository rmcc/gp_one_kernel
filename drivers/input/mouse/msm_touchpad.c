...
MODULE_DESCRIPTION("QUALCOMM MSM Touchpad driver");
...
static int __exit touchpad_remove(struct i2c_client *client)
{
	...
	gpio_free(dd->touchpad_suspend_gpio);
	gpio_free(dd->touchpad_gpio);
	kfree(dd);
			  ...
			  const struct i2c_device_id *id)
{
	...
	rc = gpio_request(dd->touchpad_gpio, "msm_touchpad_irq");
	if (rc)
		goto probe_free_exit;
	rc = gpio_request(dd->touchpad_suspend_gpio, "msm_touchpad_suspend");
	if (rc)
		goto probe_gpioirq_exit;
