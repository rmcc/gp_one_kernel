...
static void Power_off_charging()
{
	...
	qcom_gpio_direction_output(GPIO_CHR_DET,0);
	while(1)
  {			
...
///+++ FIH_ADQ +++ 
	...
	rc = qcom_gpio_request(USBSET, "USBSET");
	if (rc)	printk(KERN_ERR "%s: USBSET setting failed! rc = %d\n", __func__, rc);
	gpio_set_value(USBSET,0);
	qcom_gpio_free(USBSET);
        //Turn off Charger IC
	rc = qcom_gpio_request(33, "CHR_EN");
	if (rc)	printk(KERN_ERR "%s: CHR_EN setting failed! rc = %d\n", __func__, rc);
	gpio_set_value(33,1);////qcom_gpio_direction_output(33,1);
	qcom_gpio_free(33);
///+++ FIH_ADQ +++
    check_USB_type =0;
				...
				if (usb_vbus_is_on(ui)) {
				  ...
				  writel(readl(USB_USBCMD) & ~USBCMD_RS, USB_USBCMD);				  
					...
					//disable_irq(ui->irq);
                                        /*+++FIH_ADQ+++*/
                                        //Turn off Charger IC
					rc = qcom_gpio_request(CHR_EN, "CHR_EN");
					if (rc)	printk(KERN_ERR "%s: CHR_EN setting failed! rc = %d\n", __func__, rc);
					gpio_set_value(CHR_EN,1);////qcom_gpio_direction_output(33,1);
...
static void usb_chg_set_type(struct usb_info *ui)
{
	...
	rc = qcom_gpio_request(USBSET, "USBSET");
	if (rc)	printk(KERN_ERR "%s: USBSET setting failed! rc = %d\n", __func__, rc);
	rc = qcom_gpio_request(CHR_1A, "CHR_1A");
