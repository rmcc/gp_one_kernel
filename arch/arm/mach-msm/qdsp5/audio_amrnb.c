...
static int audamrnb_enable(struct audio *audio)
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_enable()\n");
...
{
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_disable()\n");
		...
		uint32_t *payload)
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_update_pcm_buf_entry()\n");
			      ...
			      void (*getevent) (void *ptr, size_t len))
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: audplay_dsp_event()\n");
...
static void audamrnb_dsp_event(void *private, unsigned id, uint16_t *msg)
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_dsp_event(id=%d)\n", id);
...
static int auddec_dsp_config(struct audio *audio, int enable)
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: auddec_dsp_config()\n");
...
static void audpp_cmd_cfg_adec_params(struct audio *audio)
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: audpp_cmd_cfg_adec_params()\n");
...
static void audpp_cmd_cfg_routing_mode(struct audio *audio)
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: audpp_cmd_cfg_routing_mode()\n");
				       ...
				       unsigned idx, unsigned len)
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: audplay_dsp_send_data_avail()\n");
...
static void audamrnb_buffer_refresh(struct audio *audio)
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_buffer_refresh()\n");
...
static void audamrnb_config_hostpcm(struct audio *audio)
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_config_hostpcm()\n");
...
static void audamrnb_send_data(struct audio *audio, unsigned needed)
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_send_data()\n");
...
{
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_flush()\n");
...
static void audamrnb_flush_pcm_buf(struct audio *audio)
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_flush_pcm_buf()\n");
...
static void audamrnb_ioport_reset(struct audio *audio)
{
	 ...
	 */
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_ioport_reset()\n");
		...
		unsigned long arg)
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_ioctl(cmd=%d)\n", cmd);
			  ...
			  loff_t *pos)
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_read()\n");
			   ...
			   size_t count, loff_t *pos)
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_write()\n");
...
static int audamrnb_release(struct inode *inode, struct file *file)
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_release()\n");
...
static int audamrnb_open(struct inode *inode, struct file *file)
{
	...
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_open()\n");
...
{
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_init()\n");
...
{
	if(LOG_CMCS) pr_info("audio_amrnb: audamrnb_exit()\n");
