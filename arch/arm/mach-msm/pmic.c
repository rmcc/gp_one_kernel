...
#else /* CONFIG_MACH_ADQ */
/* ADQ has these values shifted by 1... */
enum {
   LIB_RPC_GLUE_CODE_INFO_REMOTE_PROC = 0,
   LP_MODE_CONTROL_PROC,
   VREG_SET_LEVEL_PROC,
   VREG_PULL_DOWN_SWITCH_PROC,
   ...
   RTC_DISABLE_ALARM_PROC,
   RTC_GET_ALARM_TIME_PROC,
   RTC_GET_ALARM_STATUS_PROC,
   RTC_SET_TIME_ADJUST_PROC,
   RTC_GET_TIME_ADJUST_PROC,
   SET_LED_INTENSITY_PROC,
   FLASH_LED_SET_CURRENT_PROC,
   FLASH_LED_SET_MODE_PROC,
   FLASH_LED_SET_POLARITY_PROC,
   SPEAKER_CMD_PROC,
   SET_SPEAKER_GAIN_PROC,
   VIB_MOT_SET_VOLT_PROC,
   VIB_MOT_SET_MODE_PROC,
   VIB_MOT_SET_POLARITY_PROC,
   VID_EN_PROC,
   VID_IS_EN_PROC,
   VID_LOAD_DETECT_EN_PROC,
   MIC_EN_PROC,
   MIC_IS_EN_PROC,
   MIC_SET_VOLT_PROC,
   MIC_GET_VOLT_PROC,
   SPKR_EN_RIGHT_CHAN_PROC,
   SPKR_IS_RIGHT_CHAN_EN_PROC,
   SPKR_EN_LEFT_CHAN_PROC,
