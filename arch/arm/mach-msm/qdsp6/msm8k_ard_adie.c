...
s32 adie_init(void)
{
	...
	/*
	pm_set_speaker_gain(PM_SPKR_GAIN_PLUS04DB);
	pm_mic_set_volt(PM_MIC_VOLT_1_80V);
	pm_speaker_cmd(PM_SPKR_ENABLE)
	*/
...
s32 adie_dinit(void)
{
	...
	/*pm_speaker_cmd(PM_SPKR_DISABLE);*/
							       ...
							       u32 dev_id)
{
	...
	if (adie_state.adie_path_type[dev_type].enabled == ADIE_TRUE) {
		...
		if (path_type == ADIE_CODEC_TX) {
			/*pm_mic_en(PM_OFF_CMD);*/
		} else if ((path_type == ADIE_CODEC_RX) &&
			(dev_id == CAD_HW_DEVICE_ID_SPKR_PHONE_MONO)) {

			/*pm_speaker_cmd(PM_SPKR_OFF);*/
			/*pm_spkr_en_left_chan(FALSE);*/
		} else {
			D("ARD ADIE Loopback Device\n");
		}
