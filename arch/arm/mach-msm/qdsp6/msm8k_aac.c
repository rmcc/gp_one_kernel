		...
		unsigned int cmd, unsigned long arg)
{
	...
	/* copy_from_user() is done in the CAD code */
	...
	case AUDIO_START:
		...
		cad_write_aac_fmt.aac.sample_rate = p->cfg.sample_rate;
	...
	case AUDIO_SET_AAC_CONFIG:
		rc = copy_from_user(&p->aac_cfg, (void *)arg,
				sizeof(struct msm_audio_aac_config));
		if (rc) {
			pr_err("cad_ioctl() AUDIO_SET_AAC_CONFIG failed\n");
			break;
		}
		break;
...
static int __init msm8k_aac_init(void)
{
	...
	g_aac.cfg.sample_rate = CAD_SAMPLE_RATE_48000;
